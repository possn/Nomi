...
function home(){app.innerHTML=`<main class="screen"><div class="shell"><section class="content"><div class="top"><button class="icon-btn">☰</button><div class="avatar">PN</div></div><div class="greeting">Boa noite, Pedro 👋</div><h1 class="hero">O que te apetece<br>fazer hoje?</h1><div class="actions">${[['🍽️','Comer','primary'],['☕','Café',''],['🍸','Beber um copo',''],['🍰','Sobremesa',''],['🎲','Surpreende-me','sage']].map(([e,l,c])=>`<button class="action ${c}" onclick="start()"><span class="e">${e}</span>${l}</button>`).join('')}</div></section>${nav('home')}</div></main>`}
...
function distance(){const a=[5,10,15,20,30];app.innerHTML=`<main class="flow">${head(3,'sage')}<h1 class="title">Até onde vais?</h1><p class="sub">Tempo máximo de deslocação</p><div class="value">${state.distance} min</div><div class="slider"><input class="sage" type="range" min="0" max="4" value="${a.indexOf(state.distance)}" oninput="state.distance=[5,10,15,20,30][this.value];render()"><div class="labels"><span>5 min</span><span>10 min</span><span>15 min</span><span>20 min</span><span>30+</span></div></div><div class="flex"></div><button class="cta sage" onclick="setScreen('prefs')">Continuar</button></main>`}
...
function radius(){return Math.min(6000,Math.max(900,state.distance*120))}
...
function normalize(el,loc){
 ...
 return {id:`${el.type}-${el.id}`,name:t.name||t['name:pt']||'',cuisine:c,lat,lon,distanceKm:d,address:[t['addr:street'],t['addr:housenumber']].filter(Boolean).join(' '),score:Math.max(1,Math.min(99,Math.round(score)))}
}
...
function result(){const [f,...rest]=state.results;app.innerHTML=`<main class="results"><div class="rhead"><button class="icon-btn" onclick="setScreen('prefs')">←</button><b>Nomi encontrou para ti</b><button class="icon-btn" onclick="search()">↻</button></div><article class="maincard"><div class="badge">♥ ${f.score}%<br>match</div><h2>${f.name}</h2><div class="meta">${f.cuisine}${f.address?` · ${f.address}`:''}</div><p class="why">${why(f)}</p><div class="stats"><span>⌖ ${f.distanceKm.toFixed(1)} km</span>${f.rating?`<span>★ ${f.rating}</span>`:''}<a class="map" target="_blank" rel="noopener" href="${map(f)}">Mapa</a><button class="fav" onclick="fav('${f.id}')">${isFav(f.id)?'♥':'♡'}</button></div></article><h3 class="sectiontitle">Outras excelentes opções</h3>${rest.slice(0,5).map(r=>`<article class="alt"><div><h3>${r.name}</h3><p>${r.cuisine} · ${r.distanceKm.toFixed(1)} km${r.address?` · ${r.address}`:''}</p></div><a target="_blank" rel="noopener" href="${map(r)}">Mapa</a></article>`).join('')}<button class="cta" onclick="toast('A Nomi decidiu: ${f.name.replace(/'/g,'')}')">🎲 Decide por mim</button><div class="note">Resultados ao vivo via ${state.provider}. Não existem restaurantes pré-gravados nesta versão.</div></main>`}
